...
class ChatService:
    def __init__(self, vector_repo: VectorRepository, llm: Llm):
        self.vector_repo = vector_repo
        self.llm = llm

    async def chat(self, user_id: UUID, request: ChatRequest) -> ChatResponse:
        # Retrieval: 사용자 질문과 관련된 과거 기억(일정) 검색
        ...
        # Generation: 질문+맥락 전달
        answer = await self.llm.ask(question=request.message, context=context_str)
